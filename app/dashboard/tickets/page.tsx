...
export default async function TicketsPage({
  ...
  if (user.role === 'COORDINATOR') {
    whereClause.customer = { departmentId: (await prisma.user.findUnique({ where: { id: user.id }, select: { departmentId: true } }))?.departmentId }
  ...
  const [tickets, totalCount, agents, openTicketsCount] = await Promise.all([
    ...
    prisma.ticket.count({
      where: user.role === 'EDITOR' || user.role === 'VIEWER'
        ...
        ? { status: 'OPEN', assigneeId: user.id }
        : { status: 'OPEN' }
    })
  ])
          ...
          <div className="p-4 lg:p-8">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6 lg:mb-8 gap-4">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400 mt-1">Gestiona todos los tickets de soporte</p>
              </div>
              <CreateTicketButton />
            </div>
