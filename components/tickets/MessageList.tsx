...
export default function MessageList({ ticket, messages, currentUserId }: MessageListProps) {
  return (
    <div className="space-y-6">
      ...
      {ticket.description && (
        <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-6 shadow-sm border-l-4 border-primary-500">
      ...
      {messages.map((message) => {
        ...
        return (
          <div key={message.id} className={`${isInternal ? 'bg-yellow-50 dark:bg-yellow-900/20' : isSystem ? 'bg-gray-50 dark:bg-slate-800' : 'bg-gray-50 dark:bg-slate-800'} rounded-lg p-6 shadow-sm border border-gray-200 dark:border-slate-700`}>
            {isSystem ? (
